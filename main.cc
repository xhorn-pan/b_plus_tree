...
int main(int argc, char *argv[]) {
  if (argc != 2) {
    std::cout << "Invalid param, Usage: ./bplustree <input_filename>"
              << std::endl;
    exit(-1);
  }
  ...
  std::regex e_cmd(
      "^\\s*([Ii]ntitialize|[Ii]nsert|[Dd]elete|[Ss]earch)\\((.*)\\)\\s*$");
  ...
  while (std::getline(fi, line)) {
    ...
    if (std::regex_match(line, m_s, e_cmd)) {
      ...
      if (cmd == "Intitialize") {
        ...
        if (m < 3) {
          std::cout << "B+ tree must have a order great than 2, you give " << m
                    << std::endl;
          exit(-1);
        }
        b = new bp_tree(m);
