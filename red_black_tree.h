...
template<typename TData>
inline red_black_tree<TData>::~red_black_tree() {
    // TODO
}

template<typename TData>
void red_black_tree<TData>::print_in_order() {
...
inline void red_black_tree<TData>::right_rotate ( rb_tree_node* node ) {
     ...
     *  left_size of y_after = #nodes in b = left_size of y_before - (left_size of x_before + 1);
     */
    auto lc = node->left;
    node->left = lc->right;
    if ( lc->right != nullptr ) {
...
template<typename TData>
inline void red_black_tree<TData>::fix_parent_pointer ( rb_tree_node* old_node, rb_tree_node* new_node ) {
...
void red_black_tree<TData>::fixup_tree ( rb_tree_node* node ) {
    ...
    if ( ( node->left != nullptr ) && ( node->left->color == node_color::red ) ) {
        p = node->left;
        rotateType &= ~1;
    } else if ( ( node->right != nullptr ) && ( node->right->color == node_color::red ) ) {
        p = node->right;
        rotateType |= 1;
    }
